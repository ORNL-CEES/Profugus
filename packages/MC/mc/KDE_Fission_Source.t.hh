//---------------------------------*-C++-*-----------------------------------//
/*!
 * \file   MC/mc/KDE_Fission_Source.t.hh
...
#ifndef MC_mc_KDE_Fission_Source_t_hh
#define MC_mc_KDE_Fission_Source_t_hh
...
KDE_Fission_Source<Geometry>::KDE_Fission_Source(RCP_Std_DB     db,
    ...
    : Base(db, geometry, physics, rng_control)
{
    ...
    VALIDATE(kde_db.template isType<std::string>("kernel_type"),
             "KDE Kernel type was not specified in input");

    // Get the type of KDE kernel
    const std::string &kernel_type =
        kde_db.template get<std::string>("kernel_type");

    // Get the bandwidth coefficient
    double coeff    = kde_db.template get<double>("bnd_coeff", 1.06);
    double exponent = kde_db.template get<double>("bnd_exp", -0.20);

    if (kernel_type == "fission_rejection")
    {
        // Instantiate the resample kernel
        d_kernel = std::make_shared<Axial_KDE_Kernel_t>(
            geometry, physics, Axial_KDE_Kernel_t::FISSION_REJECTION,
            coeff, exponent);
    }
    else if (kernel_type == "cell_rejection")
    {
        // Instantiate the resample kernel
        d_kernel = std::make_shared<Axial_KDE_Kernel_t>(
            geometry, physics, Axial_KDE_Kernel_t::CELL_REJECTION,
            coeff, exponent);
    }
    else
    {
 ...
 */
template<class Geometry>
typename KDE_Fission_Source<Geometry>::SP_Particle
KDE_Fission_Source<Geometry>::get_particle()
{
 ...
 */
template<class Geometry>
typename KDE_Fission_Source<Geometry>::Fission_Site&
KDE_Fission_Source<Geometry>::sample_fission_site(RNG &rng) const
{
...
#endif // MC_mc_KDE_Fission_Source_t_hh

//---------------------------------------------------------------------------//
// end of MC/mc/KDE_Fission_Source.t.hh
//---------------------------------------------------------------------------//
