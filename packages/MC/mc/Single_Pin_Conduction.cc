//---------------------------------*-C++-*-----------------------------------//
/*!
 * \file   MC/mc/Single_Pin_Conduction.cc
 * \author Steven Hamilton
 * \date   Thu Jul 26 09:06:36 2018
 * \brief  Single_Pin_Conduction class definitions.
        ...
        std::vector<double>&       fuel_temp)
{
    ...
    for (int r = num_rings_fuel; r < num_rings; ++r)
        radius[r+1] = radius[r] + dr;
    CHECK(profugus::soft_equiv(radius[num_rings], d_clad_radius));
    ...
    double fuel_area = std::accumulate(area.begin(),
                                       ...
                                       0.0);
    CHECK(profugus::soft_equiv(fuel_area, pi * d_fuel_radius * d_fuel_radius));

    double total_area = std::accumulate(area.begin(), area.end(), 0.0);
    CHECK(profugus::soft_equiv(total_area, pi * d_clad_radius * d_clad_radius));
...
//---------------------------------------------------------------------------//
// end of MC/mc/Single_Pin_Conduction.cc
//---------------------------------------------------------------------------//
