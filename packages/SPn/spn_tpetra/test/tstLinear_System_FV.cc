//----------------------------------*-C++-*----------------------------------//
/*!
 * \file   spn_tpetra/test/tstLinear_System_FV.cc
...
{
  protected:
    typedef profugus::tpetra::Linear_System_FV Linear_System;
...
TEST_F(MatrixTest, SP7_3Grp_Refl_Graph)
{
    ...
    EXPECT_EQ(4 * 3 * data->num_cells(), g->getGlobalNumRows());
    EXPECT_EQ(4 * 3 * data->num_cells(), g->getGlobalNumCols());
}
...
TEST_F(MatrixTest, SP5_3Grp_Refl_Matrix)
{
    ...
    // make the matrix
    system->build_Matrix();
    Teuchos::RCP<Element_Matrix_t> A =
        Teuchos::rcp_dynamic_cast<Element_Matrix_t>(system->get_Operator());
    EXPECT_TRUE(A->isStorageOptimized());

    EXPECT_EQ(3 * 3 * mesh->num_cells(), A->getNodeNumRows());
    EXPECT_EQ(3 * 3 * data->num_cells(), A->getGlobalNumRows());
    EXPECT_EQ(3 * 3 * data->num_cells(), A->getGlobalNumCols());
}
...
TEST_F(MatrixTest, SP1_2Grp_Refl_Matrix)
{
    ...
    EXPECT_EQ(mesh->num_cells() * 2, x.getLocalLength());
    EXPECT_EQ(mesh->num_cells() * 2, y.getLocalLength());
...
TEST_F(MatrixTest, SP3_2Grp_Refl_Matrix)
{
    ...
    EXPECT_EQ(2 * 2 * mesh->num_cells(), A.getNodeNumRows());
    EXPECT_EQ(2 * 2 * data->num_cells(), A.getGlobalNumRows());
    EXPECT_EQ(2 * 2 * data->num_cells(), A.getGlobalNumCols());

    // make a vector
    Vector_t x(system->get_Map());
    Vector_t y(system->get_Map());
    EXPECT_EQ(mesh->num_cells() * 2 * 2, x.getLocalLength());
    EXPECT_EQ(mesh->num_cells() * 2 * 2, y.getLocalLength());
        ...
        for (int j = 0; j < mesh->num_cells_dim(J); ++j)
        {
            for (int i = 0; i < mesh->num_cells_dim(I); ++i)
            {
                ...
                for (int n = 0; n < 2; ++n)
                {
                    ...
                    x_data[0 + n * num_groups + local * num_groups * 2] =
                        gv + 0.1;
                    x_data[1 + n * num_groups + local * num_groups * 2] =
                        gv + 0.2;
                }
            }
        }
...
TEST_F(MatrixTest, SP3_2Grp_Vac_Graph)
{
    ...
    EXPECT_EQ(2 * 2 * (mesh->num_cells() + nf), g->getNodeNumRows());
    EXPECT_EQ(2 * 2 * (data->num_cells() + 96), g->getGlobalNumRows());
    EXPECT_EQ(2 * 2 * (data->num_cells() + 96), g->getGlobalNumCols());
}
...
TEST_F(MatrixTest, SP3_2Grp_Vac_Matrix)
{
    ...
    Vector_t x(system->get_Map());
    Vector_t y(system->get_Map());
    EXPECT_EQ((mesh->num_cells()+nf) * 2 * 2, x.getLocalLength());
    EXPECT_EQ((mesh->num_cells()+nf) * 2 * 2, y.getLocalLength());
...
TEST_F(MatrixTest, SP7_3Grp_Refl_RHS)
{
    ...
    EXPECT_EQ(mesh->num_cells() * 4 * 3, rhs.getLocalLength());

    Teuchos::ArrayRCP<const double> rhs_data = rhs.getData(0);
...
TEST_F(MatrixTest, SP7_3Grp_Null_Fission_Matrix)
{
    ...
    Vector_t x(system->get_Map());
    Vector_t y(system->get_Map());
    EXPECT_EQ(mesh->num_cells() * 12, x.getLocalLength());
    EXPECT_EQ(mesh->num_cells() * 12, y.getLocalLength());
    y.randomize();
...
TEST_F(MatrixTest, SP7_3Grp_Fission_Matrix)
{
    ...
    if (node == 0 && nodes == 1 )
    {
        ...
        Teuchos::ArrayRCP<int> indices(8);
    ...
    for (int k = 0; k < mesh->num_cells_dim(K); ++k)
    {
        for (int j = 0; j < mesh->num_cells_dim(J); ++j)
        {
            for (int i = 0; i < mesh->num_cells_dim(I); ++i)
            {
                ...
                {
                    double gv = static_cast<double>(global + n);
                    x_data[0 + n * num_groups + local * num_groups * 4] =
                            gv + 0.1;
                    x_data[1 + n * num_groups + local * num_groups * 4] =
                        gv + 0.2;
                    x_data[2 + n * num_groups + local * num_groups * 4] =
                        gv + 0.3;
                }
            }
        }
