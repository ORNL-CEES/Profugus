//----------------------------------*-C++-*----------------------------------//
/*!
 * \file   spn_tpetra/test/tstEigenvalue_Solver.cc
...
class Inf_Med_Eigenvalue_SolverTest : public testing::Test
{
  protected:
    ...
    typedef profugus::tpetra::Eigenvalue_Solver Solver;
...
TEST_F(Inf_Med_Eigenvalue_SolverTest, 3Grp_SP1)
{
    ...
    EXPECT_EQ(mesh->num_cells() * 3, ev->getLocalLength());

    Teuchos::ArrayRCP<const double> ev_data = ev->getData(0);
