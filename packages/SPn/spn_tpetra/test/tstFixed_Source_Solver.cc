//----------------------------------*-C++-*----------------------------------//
/*!
 * \file   spn_tpetra/test/tstFixed_Source_Solver.cc
...
class Inf_Med_Solver_FVTest : public testing::Test
{
  protected:
    ...
    typedef profugus::tpetra::Fixed_Source_Solver  Fixed_Source_Solver;
...
TEST_F(Inf_Med_Solver_FVTest, 3Grp_SP1)
{
    ...
    Teuchos::RCP<const Linear_System_t> system = solver->get_linear_system();
...
TEST_F(Inf_Med_Solver_FVTest, 3Grp_SP5)
{
    ...
    for (int cell = 0; cell < mesh->num_cells(); ++cell)
    {
        ...
        double phi_0 = x_data[g00] - 2.0/3.0*x_data[g01] +
                                     8.0/15.0*x_data[g02];
        double phi_1 = x_data[g10] - 2.0/3.0*x_data[g11] +
                                     8.0/15.0*x_data[g12];
        double phi_2 = x_data[g20] - 2.0/3.0*x_data[g21] +
                                     8.0/15.0*x_data[g22];
