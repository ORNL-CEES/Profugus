...
template <class T>
class BelosSolverTest : public testing::Test
{
  protected:
    ...
    typedef profugus::BelosSolver<T>   BelosSolver;
  ...
  protected:
    ...
    void SetUp()
    {
        ...
        d_db->set("max_itr",2);
        d_db->set("verbosity","high");

        // Build solver
        d_solver = Teuchos::rcp(new BelosSolver(d_db));
  ...
  protected:
    ...
    Teuchos::RCP<MV>                     d_x;
    Teuchos::RCP<MV>                     d_b;
    Teuchos::RCP<BelosSolver>            d_solver;
...
TYPED_TEST_CASE(BelosSolverTest, MyTypes);

TYPED_TEST(BelosSolverTest, basic)
{
    ...
    EXPECT_EQ( 8, this->d_iters ); // GMRES will converge in N iterations
    EXPECT_TRUE( this->d_converged );
